...
export async function set(request: http.Request, context: Context): Promise<http.Response.Like | any> {
	...
	//wait context.state.storage.delete("alarms|")
	//wait context.state.storage.delete("alarms|recurring")
	...
	await alarm.set("printAfter5", isoly.DateTime.nextSecond(now, 30))
