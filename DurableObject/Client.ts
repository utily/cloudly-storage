...
export class Client {
	...
	private async fetch<R>(
		...
		const request = http.Request.create({ url: `https://base${path}`, method, header, body })
		const response = http.Response.from(
			await this.stub.fetch(`https://base${path}`, {
				method: method,
				headers: header
		...
		http.Request.to(http.Request.create({ url: `https://base${path}`, method, header, body }))
