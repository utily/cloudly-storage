...
describe("Selection", () => {
	it("simple", async () => {
		...
		expect(cursor).toEqual({ range: { end: "2022-01-01", start: "2022-01-01" }, type: "doc" })
		const serialized = cursor ? Cursor.serialize(cursor) : undefined
		expect(serialized).toEqual("eyJyYW5nZSI6eyJzdGFydCI6IjIwMjItMDEtMDEiLCJlbmQiOiIyMDIyLTAxLTAxIn0sInR5cGUiOiJkb2MifQ")
