...
export class Storage {
	...
	async load<T>(id: string[]): Promise<Record<string, T> | undefined>
	async load<T>(prefix: { prefix: string }): Promise<Record<string, T> | undefined>
	async load<T>(id?: string | string[] | { prefix: string }): Promise<T | Record<string, T> | undefined>
	async load<T>(id?: string | string[] | { prefix: string }): Promise<T | Record<string, T> | undefined> {
		let result: T | Record<string, T> | undefined
		if (typeof id == "string") {
			...
			result = Object.fromEntries((await this.storage.list<T>(id)).entries())
		return result
	}

	async store<T extends { id: string } & Record<string, any>>(key: string, document: T): Promise<void> {
		const idIndexKey = "id/" + document.id
		await this.removeChanged(key, idIndexKey) //remove old changed index
		const changedKey = `changed/${this.truncateDateTime(document.changed)}`
	...
	async removeChanged(key: string, idIndexKey: string): Promise<void> {
		...
		const oldChangedIndex = oldDoc ? "changed/" + this.truncateDateTime(oldDoc.changed) : undefined
		oldChangedIndex &&
			(await this.storage
