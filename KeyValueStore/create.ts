...
export function create<B, V, M = any>(
	...
	return {
		...
		list: async (options?: string | ListOptions, partitionPrefix?: string): Promise<Continuable<ListItem<V, M>>> => {
			const response = await backend.list(options, partitionPrefix)
